...
public class Config implements IMixinConfigPlugin {
    ...
    private static final File configFile = new File(FMLLoader.getGamePath().toFile(), "config/fast_recipe_search.toml");
    private static final File legacyFile = new File(FMLLoader.getGamePath().toFile(), "config/fast_recipe_search.properties");
    ...
    static {
        ...
        try (CommentedFileConfig toml = CommentedFileConfig.builder(configFile)
            ...
            sync = toml.getOrElse("ingredient_sync", false);
    ...
    private static boolean applyDefaults(CommentedFileConfig toml) {
        ...
        changed |= setDefault(toml, "ingredient_sync", false, """
                Optimizes ingredient synchronization to improve client-side search performance.
    ...
    private static boolean migrateProperties(CommentedFileConfig toml) {
        ...
        setIfPresent(toml, "ingredient_sync", parseBool(props.getProperty("ingredient_sync")));
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (isEnable) {
            if (!ingredient_sync && mixinClassName.equals("io.github.nutant.fastrecipesearch.mixin.sync.IngredientMixin")) {
